...
- Guardrail/rule: Desktop-background Safe Mode actions belong in the generated `SystemTools` host profile as `HKCU\Software\Classes\DesktopBackground\Shell\SystemTools\shell\SafeModeOptions` with visible label `Power Options`. Keep it a direct desktop-only child of `System Tools`, and include the original SafeMode actions: `Boot in Safe Mode`, `Boot in Normal Mode`, `Restart`, `Shutdown`, `Sleep`, and `Log Off`. Clean the earlier `PowerMenu` key because Explorer did not render it despite correct registry readback.
