...
### Entry - 2026-05-12 (Flatten visible menu to avoid nested border artifacts)

- Date: 2026-05-12
- Problem: The second-level `Explorer` / `Apps & Windows` cascades made the native Windows context-menu borders look strange.
- Root cause: Deep nested cascades are rendered by Explorer's legacy menu UI and cannot be styled by the script or registry values.
- Guardrail/rule: Keep the visible menu as one ordered `System Tools` cascade with direct children. Preserve maintainability through `InstallerCore` profiles, stable numeric verb keys, and cleanup of both old flat and category paths.
- Files affected: `SystemToolsMenu.reg`, `Install-SystemToolsMenu.ps1`, `Install.ps1`, `app-metadata.json`, `README.md`, `CHANGELOG.md`, `PROJECT_RULES.md`, `D:\Users\joty79\scripts\InstallerCore\profiles\*.json`.
- Validation/tests run: Profile JSON parse validation passed; generated installers parser-validated; `InstallerCore` verify passed; local-source updates completed for `SystemTools`, `TakeOwnership`, `WhoIsUsingThis`, `WinAppManager`, `SystemCleanup`, and `Firewall`; HKCU registry readback confirmed ordered direct child keys and representative child labels; old category and old flat HKCU keys confirmed removed; `RefreshShell.ps1 -NoPause` completed.
