...
`Tools Summary` also shows an `Actions Needed` section for the selected row. It recommends the safest next step, such as `W` first when the local workspace is behind GitHub, `U` for GitHub update/repair of the installed copy, `I` for local install/repair, workspace review, or no action. `U`, `W`, and `I` ask for immediate `Y/N` confirmation without requiring Enter. When a selected row needs Git review, `Enter` opens a vertical WT split pane in that tool's workspace with suggested first commands. The review pane is a normal shell; close it with `exit` or `Ctrl+Shift+W`.
